package com.vivo.arcane.earth.parallax;


import com.vivo.arcane.earth.basic.OfCourseUtil;
...
public class ParallaxIniReader {
    ...
    public String getProperty(String key, List<String> variants) {
        ...
        if (AllEntries.containsKey(key)) {
            ...
            if (OfCourseUtil.isNullOrEmpty(lastOne.env) && (lastOne.variants == null || lastOne.variants.size() == 0)) {
                result = lastOne.value;
            }
            // match from top to bottom
            for (LineParseResult item : bulk) {
                boolean envMatch = OfCourseUtil.isNullOrEmpty(item.env) || item.env.equals(env);
